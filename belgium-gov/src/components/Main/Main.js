...
class Main extends Component {
    ...
    requestToTheWallet(request){
        return new Promise(function (resolve, reject) {
            axios.post(API_URL + '/issue-verifiable-id/request', {
                data: request
            })
            .then(res => {
                resolve(res.data);
        ...
        return (
            <div className="App">
                <header className="App-header">
                    <p> <b>Hi</b>  {this.state.username}  </p>
                    <img src={logo} className="App-logo" alt="logo" />
                    <p>
                        Belgium Government
                    </p>
                    <Button className="view-button" variant="info" onClick={() => this.requestVerifableID()}>Request Verifiable ID</Button>
                </header>
            </div>
        );
    }
